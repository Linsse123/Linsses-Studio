                ...
                <AnimatePresence mode="wait">
                    {activeTab === 'monitor' ? (
                        <motion.div key="monitor" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2 space-y-6">
                                <div className="glass p-8 rounded-3xl border border-white/10 relative overflow-hidden h-[400px] flex flex-col justify-center">
                                    <div className="absolute top-4 left-6 flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] uppercase tracking-widest font-bold opacity-40">Servidor en Producción</span>
                                    </div>
                                    ...
                                    <div className="absolute border-t border-white/5 bottom-0 left-0 right-0 p-6 flex items-center justify-between backdrop-blur-xl">
                                        ...
                                        >
                                            ...
                                            {isExecuting ? 'Ejecutando en Servidor...' : 'Ejecutar Pipeline'}
                                        </button>
                                        ...
                                        <div className="flex gap-4">
                                            <a href={n8nUrl} target="_blank" rel="noreferrer" className="text-xs text-white/40 hover:text-white flex items-center gap-1 transition-colors">
                                                Panel n8n <ExternalLink className="w-3 h-3" />
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <QuickStat icon={<CheckCircle2 className="text-emerald-400" />} label="Estatus" value={isExecuting ? "Ejecutando" : "Estable"} />
                                    <QuickStat icon={<Activity className="text-blue-400" />} label="Tipo de Demo" value="Live Server" />
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <div className="bg-white/5 border border-white/5 p-8 rounded-3xl space-y-6 backdrop-blur-md">
                                    ...
                                    <p className="text-[#ededed]/60 leading-relaxed text-sm">
                                        Los datos filtrados de CDMX se exportan automáticamente a Google Sheets tras la ejecución.
                                    </p>
                                    <a href={sheetsUrl} target="_blank" rel="noreferrer" className="block p-5 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 rounded-2xl text-emerald-400 font-bold text-center transition-all group">
                                        Abrir Excel (Sheets)
                                        <ChevronRight className="inline ml-1 group-hover:translate-x-1 transition-transform" />
                                    </a>
                                </div>
                            </div>
                        </motion.div>
                        ...
                        <motion.div key="code" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="grid grid-cols-1 md:grid-cols-4 gap-8">
                            <div className="space-y-4">
                                {NODES_DATA.map((node, idx) => (
                                    <button
                                        ...
                                        className={`w-full text-left p-6 rounded-2xl border transition-all ${selectedNode === idx ? 'bg-blue-500/10 border-blue-500 text-blue-400' : 'bg-white/5 border-white/5 hover:border-white/10'
                                            }`}
                                    >
                            ...
                            <div className="md:col-span-3 bg-white/5 border border-white/5 p-8 rounded-3xl space-y-8 backdrop-blur-md overflow-hidden">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h2 className="text-3xl font-bold mb-1">{NODES_DATA[selectedNode].name}</h2>
                                        <p className="text-[#ededed]/60">{NODES_DATA[selectedNode].description}</p>
                                    </div>
                                    <div className="px-3 py-1 bg-white/5 rounded-full text-[10px] uppercase font-bold tracking-widest opacity-40">
                                        Source Code
                                    </div>
                                </div>
                                <div className="bg-black/50 p-6 rounded-2xl font-mono text-sm border border-white/5 overflow-x-auto whitespace-pre leading-relaxed text-blue-300 shadow-inner">
